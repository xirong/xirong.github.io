...
(function () {
    ...
    const AUDIO_FALLBACK = {
        ...
        'arriving-langfang': '前方到站，廊坊。',
        'langfang-stop': '廊坊站到了。',
        'langfang-depart': '廊坊站开车，下一站，终点站北京南。',
        'arriving-beijing': '前方终点站，北京南。',
    ...
    function phaseToStatusText() {
        if (phase === Phase.DRIVING) {
            ...
            if (langfangState === 'stopped') return '停靠廊坊';
            if (langfangState === 'stopping') return '进站减速';
            if (langfangState === 'departing') return '廊坊发车';
            if (langfangState === 'passed') return '驶向北京南';
            return '行驶中';
        }
    ...
    function addRouteOverlay(svgEl, bjEl, sdEl, forMiniMap) {
        ...
        addCityMarker(svgEl, ns, dzCx, dzCy, '德州东', '', 16);
        addCityMarker(svgEl, ns, lfCx, lfCy, '廊坊', 'mid-label', -12);
        addCityMarker(svgEl, ns, bjCx, bjCy, '北京南', 'dest-label', -12);
    ...
    function initStationScene() {
        ...
        showHint('德州东站发车前，先上 16 车，找到 10F！', 3600);
    ...
    function createStationDezhouEast() {
        ...
        lctx.fillText('复兴号 G746 次列车', 512, 94);
        lctx.font = 'bold 46px "Noto Sans SC", sans-serif';
        lctx.fillStyle = '#ffe066';
        lctx.fillText('德州东 → 廊坊 → 北京南', 512, 166);
    ...
    function createFuxingCar(isHead, isTail, length, width, height, carNum) {
        ...
        if (isHead) {
            ...
            addG746Label(group, width, height, length, true);
    ...
    function addG746Label(group, carWidth, carHeight, carLength, compact) {
        const c = document.createElement('canvas');
        c.width = 1024; c.height = 256;
        const ctx = c.getContext('2d');
        ctx.clearRect(0, 0, c.width, c.height);
        ...
        const txt = compact ? 'G746 次列车' : 'G746';
    ...
    function animateDriving(timestamp) {
        ...
        if (langfangState === 'approaching') {
            ...
            langfangStopTimer = 0;
            $('stationLabel').textContent = '廊坊站';
            $('stationLabel').classList.add('show');
            playAudio('langfang-stop');
            showHint('廊坊站短暂停靠中...', 1500);
    ...
    function startArrivedPhase() {
        ...
        $('arrivalStats').textContent =
            '全程 ' + TOTAL_DISTANCE + ' 公里，途经廊坊，用时 ' + min + ' 分 ' + sec + ' 秒';
